from lstm.preproc import prepro
from lstm.lstm import ABSA_Lstm
from lstm.basic import get_score
...
def train(config):
    ...
    # init model
    model = ABSA_Lstm(dim_word=config.dim_word, dim_hidden=config.dim_hidden, num_classification=config.num_class,
                      maxlen=config.sent_limit, wordemb=wordemb, device=device)
