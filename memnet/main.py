from memnet.preproc import prepro
from memnet.memnet import MemNet
from memnet.basic import get_score
from memnet.dataset import get_loader
from memnet.config import config
import torch
...
def train(config):
    ...
    # init model
    model = MemNet(dim_word=config.dim_word, num_class=config.num_class, num_hop=config.cpt_num, wordmat=wordemb,
                   device=device)
