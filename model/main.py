from model.preproc import prepro
# from model.cnn_gate import Model
# from model.bigru_gate import Model
# from model.bigru_attention import Model
from model.bigru_vp_attention import Model
# from model.model import ConvAttention as Model
from model.basic import get_score
...
def train(config):
    ...
    # init model
    # model = Model(dim_word=config.dim_word, num_channel=config.num_channel, ks=config.kernel_size,
    #               num_class=config.num_class, wordmat=wordemb, device=device).to(device)
    model = Model(dim_word=config.dim_word, dim_hidden=config.dim_hidden, num_class=config.num_class, wordmat=wordemb,
    ...
    for epoch in tqdm(range(config.max_epoch)):
        ...
        for i, batch_data in tqdm(enumerate(train_data)):
            model.zero_grad()
            sent_ids, lens, sent_masks, aspect_ids, aspect_lens, aspect_masks, polarity = batch_data
            sent_ids, sent_masks, aspect_ids, aspect_masks, polarity = \
                sent_ids.to(device), sent_masks.to(device), \
                aspect_ids.to(device), aspect_masks.to(device), \
                polarity.to(device)
            logit = model(sent_ids, sent_masks, aspect_ids, aspect_masks)
