...
class ClfHead(nn.Module):
    ...
    def __init__(self, clf_token, cfg, n_class):
        ...
        self.lstm = nn.LSTM(self.n_embd, 100, batch_first=True)
        nn.init.normal_(self.lstm.weight_ih_l0, std=0.02)
        nn.init.normal_(self.lstm.weight_hh_l0, std=0.02)
        nn.init.normal_(self.lstm.bias_ih_l0, std=0.02)
        nn.init.normal_(self.lstm.bias_hh_l0, std=0.02)

    def forward(self, h, x):
        clf_h, _ = self.lstm(h)
        clf_h = clf_h.contiguous().view(-1, 100)
        flat = x[..., 0].contiguous().view(-1)
        clf_h = clf_h[flat == self.clf_token, :]
