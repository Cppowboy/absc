...
class ClassificationLossCompute:
    ...
    def __call__(self, X1, X2, Y, M1, M2, clf_logits, lm_logits=None, only_return_losses=False):
        # Language modeling loss
        if lm_logits is not None:
            x_shifted = X1[:, 1:, 0].contiguous().view(-1)
            M1 = M1.view(-1, M1.size(-1))
            lm_losses = self.lm_criterion(lm_logits, x_shifted)
            lm_losses = lm_losses.view(X1.size(0), X1.size(-2) - 1)
            lm_losses = lm_losses * M1[:, 1:]
            lm_losses = lm_losses.sum(1) / torch.sum(M1[:, 1:], 1)
        # Classification loss
        clf_losses = self.clf_criterion(clf_logits, Y)
        if only_return_losses:
            return (clf_losses, lm_losses) if lm_logits is not None else clf_losses
