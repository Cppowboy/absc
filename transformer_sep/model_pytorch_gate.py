...
class ClfHead(nn.Module):
    ...
    def __init__(self, clf_token, cfg, n_class):
        ...
        nn.init.normal_(self.linear.weight, std=0.02)
        nn.init.normal_(self.linear.bias, 0)
        self.conv1 = nn.Conv1d(cfg.n_embd, cfg.n_embd, 3)
        self.conv2 = nn.Conv1d(cfg.n_embd, cfg.n_embd, 3)
        self.aspect_linear = nn.Linear(cfg.n_embd, cfg.n_embd)

    def forward(self, h1, h2, x1, x2):
        aspect = torch.mean(h2, 1)
        a = F.tanh(self.conv1(h1.transpose(1,2)))
        b = h1 + self.aspect_linear(aspect).unsqueeze(1)
        b = F.relu(self.conv2(b.transpose(1, 2)))
        h = a * b
        clf_h = F.max_pool1d(h, h.size(2)).squeeze(2)
...
class DoubleHeadModel(nn.Module):
    ...
    def __init__(self, cfg, clf_token, task_head_type, vocab=40990, n_ctx=512, n_asp=10):
        super(DoubleHeadModel, self).__init__()
        self.transformer = TransformerModel(cfg, vocab=vocab, n_ctx=n_ctx)
        self.asp_transformer = TransformerModel(cfg, vocab=vocab - n_ctx + n_asp, n_ctx=n_asp)
    ...
    def forward(self, x1, x2):
        h1 = self.transformer(x1)
        h2 = self.asp_transformer(x2)
        lm_logits = self.lm_head(h1)
        task_logits = self.task_head(h1, h2, x1, x2)
...
class MyAttention(nn.Module):
    def __init__(self, d):
        super().__init__()
        self.W = nn.Parameter(torch.FloatTensor(d, d))
    ...
    def forward(self, q, k, v):
        ...
        :return: batch, d
        '''
        a = torch.matmul(q, self.W).unsqueeze(1)  # batch, 1, d
        alpha = torch.bmm(a, k.transpose(1, 2))  # batch, 1, l
        alpha = F.softmax(alpha, 1)  # batch, 1, l
        out = torch.bmm(alpha, v).squeeze(1)  # batch, d
        return out
